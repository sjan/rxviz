...
`,
  ...
  'combining-events': {
    name: 'Merging Clicks',
    code: `
// This example visualizes merging clicks. Each click increments the progress meter
const { fromEvent, merge, of, interval, from} = Rx;
const { map, flatMap, delay, takeUntil, throttle} = RxOperators;
...
buttonOne.innerHTML = "Increment";
buttonTwo.innerHTML = "Increment Throttled";
progressMeter.max = 100
progressMeter.value = 0

output.prepend(buttonOne);
output.prepend(progressMeter);
output.prepend(buttonTwo);

const clickStream = fromEvent(buttonOne, 'click')
  .pipe(
    map(e => 1)
  )

const throttledClickStream = fromEvent(buttonTwo, 'click')
  .pipe(
    map(e => 5),
    throttle(() => interval(ONE_SECOND)),
  )

const incrementSubscription = merge(clickStream, throttledClickStream)
  .subscribe(
    clickValue => { progressMeter.value += clickValue }
  );

of(clickStream, throttledClickStream)
`,
    timeWindow: 10000
  },
  custom: {
